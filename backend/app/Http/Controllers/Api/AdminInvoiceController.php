...
use App\Services\CompanyRecurringInvoiceLinesService;
...
class AdminInvoiceController extends Controller
{
    ...
    public function availableServices(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => $rows->map(fn (Service $s) => [
                ...
                'is_recurring' => $s->recurring_service_id !== null,
            ])->all(),
        ]);
    }
    ...
    public function update(Request $request, Invoice $invoice): AdminInvoiceResource|JsonResponse
    {
        ...
        DB::transaction(function () use ($invoice, $totals, $cid, $mergedIds, $periodYear, $periodMonth) {
            ...
            $invoice->save();
            $invoice->services()->sync($mergedIds);
        });
        ...
        $removedIds = array_values(array_diff($prevServiceIds, $mergedIds));
        if ($removedIds !== []) {
