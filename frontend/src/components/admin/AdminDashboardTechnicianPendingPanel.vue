...
function pillClass(status) {
  ...
  if (s === 'activo') return 'border-amber-400/40 text-amber-200'
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dash-tech-pending-title"
      >
        <header class="flex shrink-0 items-start justify-between gap-3 border-b border-slate-700/80 px-5 py-4">
          <div>
            <h2 id="dash-tech-pending-title" class="text-lg font-bold text-white">
              Pendiente de pago a técnicos
            </h2>
            <p class="mt-1 text-xs text-slate-400">
              {{ titleSuffix }} · referencia por líneas (hasta 100). Marca el pago desde el detalle.
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-4 py-3">
          ...
          <ul v-else-if="rows.length" class="flex flex-col gap-3">
            <li
              v-for="s in rows"
              :key="s.id"
              class="rounded-xl border border-amber-900/40 bg-[#1e2532] p-4 shadow-md shadow-black/20"
            >
              <div class="mb-2 flex flex-wrap items-start justify-between gap-2">
                <div class="min-w-0">
                  <button
                    type="button"
                    class="font-mono text-sm font-semibold text-amber-200 underline decoration-amber-500/40 hover:text-amber-100"
                    @click="openDetail(s)"
                  >
                    {{ s.code }}
                  </button>
              ...
              <p class="mt-2 text-xs text-slate-500">Ref. técnico (pago)</p>
              <p class="text-sm font-semibold text-amber-100">{{ money(s.technician_line_total) }}</p>
            </li>
          </ul>
          <p v-else class="py-10 text-center text-sm text-slate-500">
            No hay servicios pendientes de registrar pago al técnico en este mes.
          </p>
        </div>
