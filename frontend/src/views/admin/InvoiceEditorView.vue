...
const invoiceId = computed(() => (route.params.id ? String(route.params.id) : null))
...
async function loadAvailable() {
  ...
  try {
    const data = await fetchAvailableServicesForInvoice({
      ...
      invoice_id: isEdit.value ? invoiceId.value : undefined,
    })
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      invoice.value = await fetchAdminInvoice(invoiceId.value)
...
<template>
  <section class="page">
    ...
    <form v-else class="card form" @submit.prevent="onSubmit">
      ...
      <div class="actions">
        <RouterLink class="btn secondary" to="/admin/facturas">Cancelar</RouterLink>
